...
vector<line> line_3{
    ...
    {"불광", 37.610469, 126.929887, {"연신내", "녹번"}, {90, 120}, 2, {}, {}, 0},
    {"연신내", 37.619001, 126.921008, {"구파발", "불광"}, {120, 150}, 2, {}, {}, 0},
...
vector<line> line_6{
    ...
    {"약수", 37.55434, 127.010655, {"청구", "버티고개"}, {70, 90}, 2, {3}, {108}, 1},
    {"청구", 37.560245, 127.013828, {"신당", "약수"}, {70, 70}, 2, {5}, {87}, 1},
    {"신당", 37.565972, 127.01782, {"동묘앞", "청구"}, {70, 70}, 2, {2}, {202}, 1},
    {"동묘앞", 37.572627, 127.016429, {"창신", "신당"}, {60, 70}, 2, {1}, {80}, 1},
    ...
    {"태릉입구", 37.617983, 127.07512, {"화랑대", "석계"}, {70, 80}, 2, {7}, {38}, 1},
...
line getStation(string s, int line) {
    int k = 0;
...
bool compare_result(pair<line,pair<int,float>> a, pair<line,pair<int,float>> b) {
...
int main(){
    ...
    float lng = 0; // 경도
    for (int i = 0; i < N; i++) {
        line temp = getStation(station[i], station_line[i]);
        lat += temp.lat;
        lng += temp.lng;
    }
    lat = lat / N;
    ...
    // 중심지 근접 역 탐색
    vector<pair<line,pair<int,float>>> nearStations;
    vector<int> same_line;
    float stationD;
    float temp1, temp2;
    for (int i = 0; i < 8; i++){
        for (int j = 0; j < stationCnt[i]; j++){
            temp1 = lat - stations[i][j].lat;
            temp2 = lng - stations[i][j].lng;
            stationD = sqrt(pow(temp1, 2) + pow(temp2, 2));
            if(stationD < 0.02){
                nearStations.push_back(make_pair(stations[i][j], make_pair(i + 1, stationD)));
                cout << stationD << " -> " <<stations[i][j].name << endl;
            }
        }
    }
    cout << endl;
    sort(nearStations.begin(), nearStations.end(), compare_result);
    for(int i = 0; i < nearStations.size(); i++){
        cout << "[" << nearStations[i].second.first << "," << nearStations[i].first.name<< "]";
    }
    cout << endl <<endl;
    ...
    int count_line;
    for(int i = 0; i < N; i++){
        lines.insert(station_line[i]);
    }
    count_line = lines.size();
    int count_station = nearStations.size();
    int value[count_station] = {0};
    int value_max = 0;
    for(int i = 0; i < count_station; i++){
        if(lines.find(nearStations[i].second.first) != lines.end()){
            value[i]++;
        }
        for(int j = 0; j < nearStations[i].first.transfer_num; j++){
            if(lines.find(nearStations[i].first.transfer[j]) != lines.end()){
                value[i]++;
            }
        }
    ...
    for(int i = 0; i < nearStations.size(); i++){
        if(value[i] == value_max){
            cout << "추천 역: [" << nearStations[i].second.first << "," << nearStations[i].first.name<< "]";
        }
        break;
    }
